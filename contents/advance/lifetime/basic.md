...
- `'a: 'b`，是生命周期约束语法，跟泛型约束非常相似，用于说明 `'a` 不会死（被丢弃）在 `'b` 之前
...
## 静态生命周期
在 Rust 中有一个非常特殊的生命周期，那就是 `'static`，拥有该生命周期的引用可以在程序挂掉之前都不会...，不是，（各位大佬的程序都可以长久稳定运行，`doge` 手动狗头保命）是程序安全退出, 拥有该生命周期的引用可以在程序退出之前不会被丢弃。
