...
- 不要抱着试一试的心态去试一试，否则是浪费时间和消耗学习激情。在 [Stack Overflow 2022 开发者调查](https://survey.stackoverflow.co/2022)中，Rust 已连续七年荣获“最受喜爱语言”的桂冠，它当然不仅仅是值得试一试 :)
- 深入学习一本好书或教程
...
同时也不要忽略编译器给出的警告信息（warnings）。`cargo check` 会显示编译器警告，额外的 Clippy lint 则需要通过 `cargo clippy` 检查；这些提示不仅仅包含代码风格，甚至包含了一些隐藏很深的错误！至于这些错误为何不是 `error` 形式出现，随着学习的深入，你将逐渐理解 Rust 的各种设计选择，包括这个问题。
