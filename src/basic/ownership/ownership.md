...
之前提到过，本章会用 `String` 作为例子，因此这里会进行一下简单的介绍，具体的 `String` 学习请参见 [String 类型](https://beatai.org/rust-course/basic/compound-type/string-slice)。
...
例如，字符串是需要程序运行时，通过用户动态输入然后存储在内存中的，这种情况，字符串字面值就完全无用武之地。 为此，Rust 为我们提供动态字符串类型: `String`，它拥有一块分配在堆上的缓冲区，因此可以动态伸缩，也就能存储在编译时大小未知的文本。
...
`::` 是一种调用操作符，这里表示调用 `String` 类型中的 `from` 关联函数。`String` 可以保存运行时产生的文本，将它声明为可变后，还可以继续追加内容：
...
这段代码并没有发生所有权的转移，原因很简单：代码首先将 `5` 绑定到变量 `x`，接着**拷贝** `x` 的值赋给 `y`，最终 `x` 和 `y` 都等于 `5`。这是因为 `i32` 是固定大小的简单类型，并且实现了 `Copy` 特征，赋值时会自动拷贝它的值。

因为 `i32` 实现了 `Copy` 特征，所以这里不需要转移所有权；复制的只是一个 `i32` 值，也不涉及堆内存。

> 可能有同学会有疑问：这种拷贝不消耗性能吗？一个 `i32` 只有 4 个字节，复制它的开销很小。上一章介绍的整数、浮点数、布尔值和字符等基本类型都实现了 `Copy` 特征，赋值时会像上面的代码一样直接复制值。
...
此时，可能某个大聪明( 善意昵称 )已经想到了：嗯，上面一样，把 `s1` 的内容拷贝一份赋值给 `s2`，实际上，并不是这样。对于实现了 `Copy` 特征的类型，Rust 会在赋值时自动拷贝，而 `String` 没有实现 `Copy`，因此这里会发生所有权转移。

实际上，在这段代码中，`String` 变量 `s1` 的**堆指针**、**字符串长度**和**字符串容量**都存储在栈上。堆指针指向真正存放字符串内容的堆内存；长度表示字符串内容已经占用了多少字节，容量表示已经从内存分配器申请了多少字节。`s1` 在栈上占用的空间是固定的，堆上的字符串内容则可以随着程序运行而增长。如果你有 Go 语言的经验，这里的长度和容量应该很好理解。

栈上的 `s1` 和堆上的字符串内容，可以用下面这张图来表示：
...
1. 拷贝 `String` 的这三部分和存储在堆上的字节数组
   如果该语句拷贝所有数据(深拷贝)，就需要另外分配一块堆内存，再把字符串的全部字节复制过去，字符串越长，复制的开销越大

2. 只拷贝 `String` 的这三部分
   这样不需要复制堆上的字符串内容，但是会带来新的问题。还记得我们之前提到的所有权规则吧？其中有一条就是：**一个值只允许有一个所有者**，而现在这个值（堆上的真实字符串数据）有了两个所有者：`s1` 和 `s2`。
...
由于 Rust 禁止你继续使用已经移动的值，你会看到以下的错误：
...
如果你在其他语言中听说过术语 **浅拷贝(shallow copy)** 和 **深拷贝(deep copy)**，那么只复制指针、长度和容量，而不复制堆上的字符串内容，听起来就像浅拷贝。但是 Rust 同时使第一个变量 `s1` 无效了，因此这个操作被称为 **移动(move)**，而不是浅拷贝。上面的例子可以解读为 `s1` 被**移动**到了 `s2` 中。那么具体发生了什么，用一张图简单说明：
...
这样就解决了我们之前的问题，`s1` 无法再被使用，只有 `s2` 拥有这块字符串数据。当 `s2` 离开作用域，它就会释放内存。 相信此刻，你应该明白了，为什么 Rust 称呼 `let a = b` 为**变量绑定**了吧？
...
首先，Rust 不会在赋值时自动调用 `clone`，因此 `String` 的普通赋值不会暗中复制整块堆数据。
...
`String::clone` 会复制字符串的全部字节；对于非空字符串，还要为副本准备独立的缓冲区，开销会随着字符串长度增加。在执行频繁的代码（热点路径）中，应该确认这份复制确实是需要的。

#### 拷贝(浅拷贝)

对于整数等实现了 `Copy` 特征的类型，赋值时会直接复制它的值。复制完成后，原变量和新变量都可以继续使用。再回到之前看过的例子：
...
原因是 `i32` 实现了 `Copy` 特征，所以 `let y = x` 会复制整数值，`x` 和 `y` 都仍然有效。对于这样的类型，调用 `clone` 得到的结果相同，但没有必要写得这么复杂。

Rust 有一个叫做 `Copy` 的特征，整数等简单值类型都实现了它。如果一个类型拥有 `Copy` 特征，一个旧的变量在被赋值给其他变量后仍然可用，也就是赋值的过程即是拷贝的过程。

那么什么类型是可 `Copy` 的呢？最可靠的方式是查看该类型的文档。下面列出一些常见的 `Copy` 类型：
