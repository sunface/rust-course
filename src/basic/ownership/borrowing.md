...
不过可变引用并不是随心所欲、想用就用的，它有一个重要限制：**前一个可变引用用完之前，不能再用新的可变引用访问同一份数据**：
...
`r1` 和 `r2` 虽然处于同一个花括号作用域，但 `r1` 在第一个 `println!` 后就不会再被使用，因此它的借用已经结束，接下来可以创建 `r2`。
...
> 注意，引用 `r1`,`r2`,`r3` 对 `s` 的借用从创建开始，一直持续到它最后一次使用的地方 `println!(....)`。变量本身的作用域仍然会持续到花括号 `}`。

Rust 的借用检查一直在改进。早期的时候，借用会持续到变量作用域结束，这对日常使用带来了很大的困扰，你必须非常小心的去安排可变、不可变变量的借用，免得无法通过编译，例如以下代码：
...
fn main() {
   let mut s = String::from("hello");
    ...
    let r2 = &s;
    println!("{} and {}", r1, r2);
    // 新编译器中，r1、r2 的借用在这里结束

    let r3 = &mut s;
    println!("{}", r3);
} // 老编译器中，r1、r2、r3 的借用在这里结束
  // 新编译器中，r3 的借用在这里结束
```

在老版本的编译器中（Rust 1.31 前），将会报错，因为 `r1` 和 `r2` 的借用在花括号 `}` 处结束，那么 `r3` 的借用就会触发 **无法同时借用可变和不可变** 的规则。

但是在新的编译器中，该代码将顺利通过，因为编译器能够在引用最后一次使用后结束借用。因此 `r1` 和 `r2` 的借用在第一个 `println!` 后就结束了，此时 `r3` 可以顺利借用到可变引用。

#### NLL

这种根据引用的实际使用位置判断借用何时结束的分析，叫做 **Non-Lexical Lifetimes(NLL)**。它能找到引用在作用域(`}`)结束前就不再被使用的位置。
