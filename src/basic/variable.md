...
Rust 的变量在默认情况下是**不可变的**。前文提到，这是 Rust 团队为我们精心设计的语言特性之一，让我们编写的代码更安全，意图也更清晰。当然你可以通过 `mut` 关键字让变量变为**可变的**，让设计更灵活。
...
但是可变性也非常重要。需要频繁更新大型数据结构时，我们可以直接修改其中的数据，而不必每次都重新构造整个数据结构，使用起来也更加直接。
...
选择可变还是不可变，更多的还是取决于你的使用场景。不可变可以带来安全性和清晰性，可变变量则更灵活。性能取决于数据结构、使用方式和编译器优化，不能仅凭是否使用 `mut` 来判断。
...
这和 `mut` 变量的使用是不同的：第二个 `let` 创建了新的绑定，因此可以改变值的类型；`mut` 则修改同一个绑定中的值，变量类型不能改变。编译器是否复用原来的存储空间属于实现细节，不能据此判断两种写法的性能。
