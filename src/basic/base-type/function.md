...
在上一章节语句和表达式中，我们有提到，在 Rust 中调用函数是一个表达式，因此我们可以把函数的返回值直接赋给变量。
...
1. `let x = plus_five(5)`，说明我们用一个函数的返回值来初始化 `x` 变量，因此侧面说明了在 Rust 中函数调用也是表达式，这种写法等同于 `let x = 5 + 5;`
...
- 函数没有显式声明返回类型时，会返回一个 `()`
- 语句块没有尾表达式时，也会返回一个 `()`，例如最后一行表达式以 `;` 结尾
