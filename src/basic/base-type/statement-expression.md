...
Rust 的函数体由一系列语句组成，最后还可以用一个表达式返回值，例如：
...
对于 Rust 语言而言，**这种基于语句（statement）和表达式（expression）的方式是非常重要的，你需要能明确的区分这两个概念**，但是对于很多其它语言而言，这两个往往无需区分。基于表达式是函数式语言的重要特征，**表达式总会产生一个值，这个值也可能是 `()`**。
...
该语句块会把最后一行 `x + 1` 的值返回。注意，`x + 1` 后面不能加分号，否则它的值就会被丢弃，整个语句块最终返回 `()`。这一点非常重要，请牢记！

最后，如果语句块末尾没有需要返回的值，它就会返回一个 [`()`](https://beatai.org/rust-course/basic/base-type/char-bool#单元类型)。
