...
假设有一个 `u8` ，它可以存放从 0 到 255 的值。那么当你将其修改为范围之外的值，比如 256，则会发生**整型溢出**。关于这一行为 Rust 有一些有趣的规则：当在 debug 模式编译并运行时，Rust 会检查整型溢出，若存在这些问题，则使程序在运行时 _panic_(崩溃,Rust 使用这个术语来表明程序因错误而退出)。
