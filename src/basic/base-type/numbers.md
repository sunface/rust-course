...
> 不仅仅是数值类型，Rust 也允许在复杂类型上定义运算符，例如在自定义类型上定义 `+` 运算符，这种行为被称为运算符重载，Rust 具体支持的可重载运算符见[附录 B](https://beatai.org/rust-course/appendix/operators#运算符)。
...
此外，`isize` 和 `usize` 的位数取决于目标平台的指针宽度：32 位目标平台上是 32 位，64 位目标平台上则是 64 位。
...
这么多类型，有没有一个简单的使用准则？答案是肯定的，Rust 整型默认使用 `i32`，例如 `let i = 1`，那 `i` 就是 `i32` 类型，因此拿不准时可以从它开始。`isize` 和 `usize` 的主要应用场景是用作集合的索引。


#### 整型溢出

假设有一个 `u8`，它可以存放从 0 到 255 的值。当运算结果超出这个范围时，比如让 255 加 1，就会发生**整型溢出**。使用 Cargo 默认的 `dev` 配置构建时，Rust 会在运行时检查整型溢出；一旦发生溢出，程序就会 _panic_（崩溃，Rust 使用这个术语来表明程序因错误而退出）。

使用 Cargo 默认的 `release` 配置构建时，溢出检查会被关闭，溢出的结果按照补码循环溢出（_two’s complement wrapping_）处理。比如在 `u8` 的情况下，255 加 1 会变成 0，依此类推。项目也可以通过 `overflow-checks` 调整这一行为，因此不要让代码依赖默认的溢出结果。
...
Rust 的 `HashMap` 数据结构，是一个 KV 类型的 Hash Map 实现，它要求键类型实现 `Eq` 和 `Hash` 特征。浮点数没有实现这两个特征，因此无法直接作为 `HashMap` 的 `Key`；作为对比，Rust 的整数类型、字符串类型、布尔类型都满足这些要求。
...
对于数学上未定义的结果，例如对负数取平方根 `(-42.1_f64).sqrt()`，会产生一个特殊的结果：Rust 的浮点数类型使用 `NaN`（not a number）来处理这些情况。

大多数包含 `NaN` 的算术运算仍会得到 `NaN`，而且 `NaN` 甚至不等于它自身，下面的代码会崩溃：
...
这些语句中的每个表达式都使用了数学运算符，并且计算结果为一个值，然后绑定到一个变量上。[附录 B](https://beatai.org/rust-course/appendix/operators#运算符) 中给出了 Rust 提供的所有运算符的列表。
...
对于移位运算，移位量不能大于或等于整数类型的位数。比如，`u8` 只有 8 位，因此最多移动 7 位；移动 8 位就会发生溢出。下面代码中的移位量是常量，所以编译器会直接报错。

如果移位量到运行时才能确定，默认的 `dev` 配置会在溢出时 _panic_，而 `release` 配置不会进行相同的检查。因此，不要让代码依赖移位溢出的结果。
...
像上面这样用于迭代的序列，常见的端点是整数或字符，因为这些类型具有明确的步进逻辑。通过比较起点和终点的大小，还可以判断该序列是否为空，例如 `5..1` 就是一个空序列。
...
- **不同整数类型之间不会隐式转换**. 例如 Rust 不会自动把一个 16 bit 整数值转换成 32 bit 整数值
