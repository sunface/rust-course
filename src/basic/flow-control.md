...
- 用 `if` 来赋值时，要保证每个分支返回的类型一样（事实上，这种说法不完全准确，见[这里](https://beatai.org/rust-course/appendix/expressions#if表达式)），此处返回的 `5` 和 `6` 就是同一个类型，如果返回类型不一致就会报错
...
为什么 `for` 后面既可以写 `collection`，也可以写 `&collection` 或 `&mut collection`？

因为 `for` 会先把 `in` 后面的内容转换成迭代器，再从中逐个取出元素。Rust 把这种“转换成迭代器”的能力称为 `IntoIterator`。你可以先把这三种写法理解为：

| 使用方法                      | 常见的对应写法                    | 所有权     |
| ----------------------------- | --------------------------------- | ---------- |
| `for item in collection`      | `collection.into_iter()`          | 转移所有权 |
| `for item in &collection`     | `collection.iter()`               | 不可变借用 |
| `for item in &mut collection` | `collection.iter_mut()`           | 可变借用   |

这里先记住：传入集合、不可变引用和可变引用，会得到三种不同的遍历方式。至于 `IntoIterator` 是如何定义的，以及 `for` 怎样完成转换，会在迭代器章节详细介绍。
...
- **性能**：使用 `collection[index]` 访问元素时，Rust 需要检查索引是否越界。直接迭代不需要手动计算索引，也就避免了这类检查。不过，对于简单的循环，编译器通常能够优化掉边界检查，因此两种写法的实际性能差异不能一概而论。
- **安全**：手动索引时，如果循环范围或索引计算写错，程序可能因越界而 _panic_。直接迭代只会取出集合中实际存在的元素，从写法上避免了这类错误。
...
但这个过程很容易出错；如果索引长度不正确会导致程序 **_panic_**。索引访问还需要进行边界检查，虽然编译器有时可以将它优化掉，但这里没有必要手动维护索引。
...
可以看出，`for` 并不需要我们手动使用索引去访问数组，因此更安全也更简洁。
