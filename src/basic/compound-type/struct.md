...
>
> 仔细回想一下[所有权](https://beatai.org/rust-course/basic/ownership/ownership#拷贝浅拷贝)那一节的内容，我们提到了 `Copy` 特征：实现了 `Copy` 特征的类型无需所有权转移，可以直接在赋值时进行
...
为了直观理解 `File` 结构体中各字段的所有权，可以参考下面的示意图：
<img alt="" src="https://pic3.zhimg.com/80/v2-8cc4ed8cd06d60f974d06ca2199b8df5_1440w.png" class="center"  />

图中的 `name` 和 `data` 分别拥有一段存放字节的缓冲区，`ptr` 表示缓冲区的内存地址。这张图用于展示所有权关系，并不表示 `String`、`Vec` 或结构体字段具有固定的内存排列。
...
还记得之前讲过的基本没啥用的[单元类型](https://beatai.org/rust-course/basic/base-type/char-bool#单元类型)吧？单元结构体就跟它很像，没有任何字段和属性，但是好在，它还挺有用。
...
此时结构体的输出跟我们创建时候的代码几乎一模一样了！当然，如果大家还是不满足，那最好还是自己实现 `Display` 特征，以向用户更美的展示你的私藏结构体。关于格式化输出的更多内容，我们强烈推荐看看这个[章节](https://beatai.org/rust-course/basic/formatted-output#debug-特征)。
