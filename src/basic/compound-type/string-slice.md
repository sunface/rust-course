...
`"Hello, world!"` 是字符串字面量，它定义的文本在编译时就已经确定，并会直接写进程序。变量 `s` 不拥有这段文本，而是通过 `&str` 借用它。由于 `&str` 是不可变引用，因此我们不能通过 `s` 修改字符串字面量。
...
理解 Rust 字符串时，要先分清两个问题：如何定义字符串数据，以及如何使用已有的字符串数据。

先看字符串数据的定义。我们可以把字符串直接写在源码中，也可以在程序运行时创建一个 `String`：
...
`"hello"` 是字符串字面量，它定义的文本在编译时就已经确定，并会直接写进程序。使用字符串字面量时，我们得到的是指向这段文本的 `&str`，因此变量 `literal` 并不拥有这些数据。

`String::from("world")` 则创建了一个 `String`，字符串数据归变量 `owned` 所有，所以它可以在程序运行时继续增长和修改。

定义好字符串数据后，再来看如何使用它们。当函数只想读取字符串内容，而不需要取得所有权时，可以使用 `&str`：
...
`show(literal)` 使用字符串字面量提供的 `&str`，`show(&owned)` 则从 `String` 中借用一个 `&str`。对于 `show` 来说，它只关心借用到的字符串内容，并不关心这些数据最初如何定义。

`&str` 中的 `str` 是 Rust 语言内置的 UTF-8 字符串类型，前面的 `&` 表示借用。实际编程中，`str` 通常以 `&str` 的形式出现；`String` 则是标准库提供的、拥有字符串数据的类型。**当 Rust 用户提到字符串时，往往指的就是 `String` 和 `&str`，它们都是 UTF-8 编码。**
...
Rust 字符串存储的是经过 UTF-8 编码的字节序列。对于 `let hello = String::from("Hola");` 这行代码来说，`Hola` 的长度是 `4` 个字节，因为 `"Hola"` 中的每个字母在 UTF-8 编码中仅占用 1 个字节，但是对于下面的代码呢？
...
对于 `String` 类型，为了支持一个可变、可增长的文本片段，需要在堆上分配一块在编译时未知大小的缓冲区来存放内容，这些都是在程序运行时完成的：

- 创建或增长字符串时，由内存分配器负责申请所需的缓冲区
- `String` 使用完成后，释放由它管理的缓冲区

`String::from` 会创建一个新的 `String`，需要的内存由它在内部完成申请。
