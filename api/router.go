...
func (router *router) route(c echo.Context) error {
	// 解析请求
	r, status, err := req.Parse(c)
	if err != nil {
		c.Set("api_id", "error_api_id")
		c.Set("service", "error_service")
		c.Set("app", "error_app")
		c.Set("error_msg", err)
		return c.JSON(http.StatusOK, g.Result{r.Rid, status, g.ParamInvalidC, err.Error(), nil})
	}
	...
	if r.Api.Status != misc.API_RELEASED {
		return c.JSON(http.StatusOK, g.Result{r.Rid, http.StatusBadRequest, manage.APIOfflineC, manage.APIOfflineE, nil})
	}
	...
	if res.Status != 0 {
		...
		return c.JSON(http.StatusOK, g.Result{r.Rid, res.Status, res.Ecode, res.Emsg, nil})
	}
	...
	case misc.BACKEND_TYPE_HTTP:
		...
		if err != nil {
			c.Set("error_msg", err)
			return c.JSON(http.StatusOK, g.Result{r.Rid, code, g.ReqFailedC, err.Error(), nil})
		}
