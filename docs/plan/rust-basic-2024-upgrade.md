...
- [x] 核对所有权、借用、引用和切片之间的过渡，只修正事实错误、前后矛盾或会误导读者的表述。
- [x] 只修正把 String 与 `&str` 的实现细节写成稳定保证的内容，保留其余示例和讲解。
- [x] 核查 Vec 扩容措辞，不把增长倍数写成 API 保证。
- [x] 核对结构体、枚举、Option、match 与 `if let` 示例，只修正无法验证或结论错误的部分。
- [x] 只有现有示例确实受影响时，才修正 Edition 2024 match ergonomics、`if let` 临时值或尾表达式临时值相关内容。
- [x] 保留“模式适用场景”和“全模式列表”的现有位置，只修正过时示例；不以内容重复为由删除有效讲解。
