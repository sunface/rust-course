...
- 共约 54 个章节文件、705 个代码围栏，其中约 523 个标记为 Rust。
...
- [ ] 新增 `rust-toolchain.toml`，固定 Rust 1.97.1、rustfmt 与 clippy。
- [ ] 记录当前 `mdbook build` 与 `mdbook test` 的结果。
- [ ] 为基础部分建立章节清单、代码块清单和失败清单。
- [ ] 统一代码围栏分类，先修复测试基础设施问题。
- [ ] 将 CI 拆为固定版本必过与最新 Stable 前瞻检查。
- [ ] 固定或显式管理 mdBook 版本，避免 CI 无提示漂移。
...
第一批工作从“阶段 0 + 阶段 1”开始：先固定工具链并让代码示例可分类验证，再完成安装、编辑器、Cargo、下载加速和 Hello World 五组入门章节。这样可以尽早建立一条可持续验证的 Rust 2024 学习入口，再向语言核心章节推进。
